...
interface MemorizationContextType {
  ...
  isLoaded: boolean
  addSet: (title: string, content: string, chunkMode?: ChunkMode, tags?: string[]) => string
  getSet: (id: string) => MemorizationSet | undefined
  updateSet: (id: string, title: string, content: string, tags?: string[]) => void
...
function loadFromStorage(): MemorizationSet[] {
  ...
  try {
    ...
    return parsed.map((set: Partial<MemorizationSet> & { id: string; title: string; content: string; createdAt: string }) => {
      ...
      const needsTagsMigration = !set.tags
      
      if (needsChunkMigration || needsProgressMigration || needsSessionStateMigration || needsRecommendationMigration || needsTagsMigration) {
...
export function MemorizationProvider({ children }: { children: ReactNode }) {
  ...
  const addSet = useCallback((title: string, content: string, chunkMode: ChunkMode = "paragraph", tags: string[] = []): string => {
    const id = generateId()
  ...
  const updateSet = useCallback((id: string, title: string, content: string, tags?: string[]) => {
    setSets((prev) => prev.map((set) => {
      if (set.id === id) {
        ...
        return {
          ...
          content,
          chunks,
          tags: tags !== undefined ? tags : set.tags,
